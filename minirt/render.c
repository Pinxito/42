...
void	render_scene(t_scene *scene)
{
	...
	prepare_render_scene(scene); // modular
	y = 0;
	while (y < HEIGHT)
	{
		x = 0;
		while (x < WIDTH)
		{
			...
			if (trace_ray(scene, ray, &hit))
				color = lambert_modele(hit, scene->light); // shading
			else
...
int	trace_ray(t_scene *scene, t_ray ray, t_hit *hit)
{
	...
	while (obj)
	{
		...
		{
			t_sphere *sphere = (t_sphere *)obj->data; // casteo exacto
			t_hit temp_hit;                           // hit temporal
			if (ray_sphere_intersect(ray, *sphere, &temp_hit)
				&& temp_hit.t < hit->t)
			{
				*hit = temp_hit; // guardamos el hit más cercano
				found = 1;
			}
		}
		obj = obj->next;
